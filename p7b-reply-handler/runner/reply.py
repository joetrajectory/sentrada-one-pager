...
Flow:
    ...
    2. draft     (best)     branch by classification, house rules injected
    3. gate      (Opus)     4b grounding logic, fact errors only, + house rules
                            one failure regenerates, a second HALTs

Drop this file into runner/ next to sentrada_runner.py. If the chain
runner's call_model() is importable it is used (same retries, same config);
otherwise a built-in Anthropic API / claude-CLI fallback is used.
"""
...
try:
    from sentrada_runner import call_model as _runner_call_model  # type: ignore
except Exception:
    _runner_call_model = None

RUNNER_DIR = Path(__file__).resolve().parent
TEMPLATE_DIR = RUNNER_DIR / "templates"

# Config keys follow the chain runner convention ("p4b": model for the
# grounding gate, etc). Override in config.json under "models".
DEFAULT_MODELS = {
    "p7b_classify": "claude-sonnet-5",
    "p7b_draft": "claude-fable-5",
    "p7b_gate": "claude-opus-4-8",  # same tier as the chain's 4b gate
}
...
def load_config():
    for p in (RUNNER_DIR / "config.json", RUNNER_DIR.parent / "config.json"):
        if p.exists():
            with open(p) as f:
                return json.load(f)
    return {}
...
def call_model(prompt, model_key, cfg):
    if _runner_call_model is not None:
        return _runner_call_model(prompt, model_key)
...
def resolve_piece(piece_arg, cfg):
    ...
    if p.is_dir():
        return p
    roots = [
        Path(cfg.get("pieces_dir", "pieces")),
        Path("."),
        RUNNER_DIR.parent,
        RUNNER_DIR.parent / "pieces",
    ]
    matches = []
    for root in roots:
        if not root.is_dir():
...
def main():
    ap = argparse.ArgumentParser(description="Sentrada P7b reply handler")
