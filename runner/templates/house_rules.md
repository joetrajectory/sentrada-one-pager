<!-- HOUSE RULES: the cross-cutting copy rules shared by every prompt that writes
or judges piece copy. Injected wherever a template contains {{house_rules}}
(load_template expands it). Edit rules HERE once, not per template. Canonical
copy mirrored in Notion (Prompts & Architecture). Format-specific rules (dry
wit, voice, word counts) stay in the format templates. -->
...
PROOF ATTRIBUTION RULE: proof from the sender profile is quoted as the sender's
overall record, never attached to this piece or format. "Pieces like these have
earned X" is banned unless the profile explicitly says this format earned it. When
proof feels stretched, omit it; confident copy without proof beats inflated copy.

LEVERAGE BAN (litigation and misfortune): never use litigation, legal disputes,
regulatory action, redundancies or executive departures as leverage or as a content
angle, even when fully public. It reads as surveillance, not research, and it reads
worst to exactly the senior recipients these pieces target. If the underlying
insight matters, carry it through something the recipient has said or published
themselves.
