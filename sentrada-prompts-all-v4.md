...
5. A direct, low-friction ask, made while they are holding the piece. One sentence naming one specific action they can take right now through the contact block the card engine prints at the foot of the card (reply, call or message the number, or the booking link). Write the ask in THIS piece's own vocabulary, tied to the question the piece raises ("Worth 20 minutes on the question the front page leaves open?" / "20 minutes to see this run against your quiet accounts?"). NEVER reuse an example ask or a stock construction verbatim: the batch collision rule applies to the card's ask line, and a previous batch shipped 3 cards with the identical ask because an example was copied. "My email and number are below." may follow as the pointer to the contact block. The moment of maximum attention is the wrong moment to ask for nothing (the recorded failure of the first generated-card batch: "the object asked for nothing, so there was no action to take"). Reframe per the CTA rule where the recipient is an influencer, not a buyer. The ask commits the sender to nothing beyond the conversation. After the ask, the follow-up may be signalled as the backup path ("If easier, I'll follow up by email this week"), never as the plan.
